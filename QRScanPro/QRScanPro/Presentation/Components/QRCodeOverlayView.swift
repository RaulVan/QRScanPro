...
struct QRCodeOverlayView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                ForEach(codes) { code in
                    // 计算实际框架尺寸
                    let frameWidth = max(code.bounds.width, 100) + touchPadding
                    let frameHeight = max(code.bounds.height, 100) + touchPadding
    ...
    // 提取处理点击的逻辑到单独函数
    private func handleTap(_ code: QRCodeResult) {
